...
export class L2AssetsStoreService extends BaseAssetsStoreService<L2AssetStoreData> {
    ...
    protected async getErc20TokensFromSavedTokens(walletAddress: string): Promise<Erc20Token[]> {
        const tokens = await this.l2LocalERC20Tokens.getAllTokensByChain(this.ethereumWalletChainManager.getCurrentChainSignal()()!);
        ...
        const results = await this.utilsHelper.runJobsConcurrently<number>(
            tokens.map((token) => () => this.getErc20BalanceFromBlockchain(token.address, token.decimals, walletAddress)),
    ...
    public async getErc20InfoFromBlockchain(tokenAddress: string, updateCurrentState?: boolean): Promise<Erc20Token> {
    ...
    protected async getErc20BalanceFromBlockchain(tokenAddress: string, tokenDecimals: number, walletAddress: string): Promise<number> {
        const balance = await ERC20Contract.getContract(tokenAddress, this.walletService).balanceOf(walletAddress);
    ...
    protected isErc20TokenSavedOnLocalStorage(tokenAddress: string): boolean {
        return !!this.l2LocalERC20Tokens.getToken(tokenAddress, this.ethereumWalletChainManager.getCurrentChainSignal()()!);
    }
}