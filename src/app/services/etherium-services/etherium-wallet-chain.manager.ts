...
export class EthereumWalletChainManager {
  private currentChain = signal<string | undefined>(
    localStorage.getItem(LOCAL_STORAGE_KEYS.CURRENT_ETHEREUM_CHAIN) ||
      undefined,
  );
  ...
  private setAllChainsByChainId(): void {
    const allChains: EIP1193ProviderChain[] = Object.values(
      environment.l2Configs,
    )
      .map((config: any) => ({
    ...
    this.allChainsByChainId = allChains.reduce(
      (acc, chain) => {
  ...
  public addChain(chain: EIP1193ProviderChain): void {
    localStorage.setItem(
      LOCAL_STORAGE_KEYS.ETHEREUM_CHAINS,
      JSON.stringify([
        ...JSON.parse(
          localStorage.getItem(LOCAL_STORAGE_KEYS.ETHEREUM_CHAINS) || '[]',
        ),
