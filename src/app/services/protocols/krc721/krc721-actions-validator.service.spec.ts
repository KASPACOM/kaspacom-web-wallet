...
describe('Krc721ActionsValidatorService', () => {
  ...
  const utils = {
    isNullOrEmptyString: (value: string | undefined | null) =>
  ...
  const krc721Api = {
    ...
    getPortfolioDetails: jasmine.createSpy('getPortfolioDetails'),
  };

  function commitRevealAction(
    protocolAction: object,
    options?: CommitRevealAction['options'],
  ): CommitRevealAction {
    return {
      actionScript: {
        type: 'kspr',
  ...
  beforeEach(() => {
    ...
    krc721Api.getTokenDetails.and.returnValue(
      of({
        result: {
          owner: walletAddress,
        },
    ...
    krc721Api.getPortfolioDetails.calls.reset();
    krc721Api.getPortfolioDetails.and.returnValue(
      of([
        {
          ticker: 'nftx',
          tokenIds: [],
  ...
  it('validates KRC721 transfer actions when token is unlisted', async () => {
    ...
    await expectAsync(
      service.validateCommitRevealAction(action, wallet),
    ).toBeResolvedTo({
      isValidated: true,
    });
    expect(krc721Api.getTokenDetails).toHaveBeenCalledWith('nftx', '12');
    expect(krc721Api.getPortfolioDetails).toHaveBeenCalledWith(
      walletAddress,
      'nftx',
    );
  });

  it('rejects KRC721 transfer actions when token is listed', async () => {
    krc721Api.getPortfolioDetails.and.returnValue(
      of([
        {
          ticker: 'nftx',
