...
export class Krc721ActionsValidatorService implements ProtocolActionsValidatorInterface {
        ...
        try {
            ...
            switch (data.op) {
                case Krc721OperationType.TRANSFER:
                    return await this.validateTransferKrc721Action(data as Krc721Transfer, wallet);
    ...
    private async validateTransferKrc721Action(krc721Command: Krc721Transfer, wallet: AppWallet): Promise<{ isValidated: boolean; errorCode?: number; }> {
    ...
    private async validateMintKrc721Action(krc721Command: Krc721Mint, wallet: AppWallet): Promise<{ isValidated: boolean; errorCode?: number; }> {
        if (this.utils.isNullOrEmptyString(krc721Command.tick)) {
            return {
                isValidated: false,
                errorCode: ERROR_CODES.WALLET_ACTION.INVALID_TICKER,
    ...
    private async validateDeployKrc721Action(krc721Command: Krc721Deploy, wallet: AppWallet): Promise<{ isValidated: boolean; errorCode?: number; }> {
        if (this.utils.isNullOrEmptyString(krc721Command.tick)) {
            return {
                isValidated: false,
                errorCode: ERROR_CODES.WALLET_ACTION.INVALID_TICKER,
        ...
        if (this.utils.isNullOrEmptyString(krc721Command.max) || !this.utils.isNumberString(krc721Command.max)) {
            return {
                isValidated: false,
