  ...
  template: `
    <div class="loading-container">
      ...
      <div class="loading-steps">
        <div class="step" [class.active]="currentProgress() >= 25">
          <div class="step-icon">
            <kc-icon 
              [iconClass]="currentProgress() >= 25 ? 'icon-check' : 'icon-clock'" 
              [size]="'sm'">
            </kc-icon>
          </div>
          <span class="step-text">Validating transaction</span>
        </div>
        ...
        <div class="step" [class.active]="currentProgress() >= 50">
          ...
          <span class="step-text">Broadcasting to network</span>
        </div>
        
        <div class="step" [class.active]="currentProgress() >= 75">
          <div class="step-icon">
            <kc-icon 
              [iconClass]="currentProgress() >= 75 ? 'icon-check' : 'icon-clock'" 
              [size]="'sm'">
            </kc-icon>
          </div>
        ...
        <div class="step" [class.active]="currentProgress() >= 100">
          ...
          <span class="step-text">Transaction complete</span>
        </div>
      </div>
    </div>
  `,
